...
$._PPP_ = {
  ...
  saveCurrentProjectLayout: function () {
    ...
    if (currentProjPanelDisplay) {
      var outFileName =
        app.project.name + "_Previous_Project_Panel_Display_Settings.xml";
          ...
          if (clipComponents) {
            ...
            if (clipComponents.numItems > 2) {
              ...
              if (blur) {
                  ...
                  if (blurriness) {
                    ...
                    if (allKeys.length) {
                      for (var k = 0; k < allKeys.length; ++k) {
                        ...
                        if (thisKey) {
                          blurriness.setInterpolationTypeAtKey(
                            thisKey,
                            4,
                            true
                          );
                ...
                $._PPP_.updateEventPanel(
                  "To make this sample code work, please apply the Gaussian Blur effect to the first clip in the first video track of the active sequence."
                );
              }
            }
          }
  ...
  autoReframeActiveSequence: function () {
    ...
    if (seq) {
      if (seq.isDoneAnalyzingForVideoEffects()) {
        ...
        if (newSequence) {
          $._PPP_.updateEventPanel(
            "Created reframed sequence: " + newName + "."
          );
        } else {
          $._PPP_.updateEventPanel(
    ...
    if (outPath) {
      ...
      if (projName) {
        ...
        if (result === true) {
          ...
          $._PPP_.updateEventPanel(
            "Failed to create project: " + projName + "."
          );
        }
      }
    }
  ...
  openProduction: function () {
    ...
    if (openProduction) {
      var closeFirst = confirm(
        "Close " + openProduction.name + "?",
        true,
        "Close open Production?"
      );
      if (closeFirst) {
        openProduction.close();
      }
    }
    var prodFolder = Folder.selectDialog(
      "Select the Production folder to open."
    );
    if (prodFolder) {
      var someProduction = app.openPrProduction(prodFolder.fsName);
      if (someProduction) {
        $._PPP_.updateEventPanel(
          "Opened production: " + someProduction.name + "."
        );
      } else {
        $._PPP_.updateEventPanel(
          "Failed to open production at path: " + prodFolder.fsName + "."
        );
      }
    }
  },
      ...
      if (numOpenProductionProjects) {
        ...
        for (var a = 0; a < numOpenProductionProjects; a++) {
          ...
          if (currentProject) {
            ...
            $._PPP_.updateEventPanel(
              currentProject.name + " is now " + postSuffix
            );
          }
        }
      } else {
    ...
    if (openProduction) {
      ...
      if (numOpenProductionProjects) {
        ...
        for (var c = 0; c < projArray.length; c++) {
          ...
          var result = openProduction.moveToTrash(
            currentProject.path,
            true,
            true
          );
  ...
  newSequenceFromProjectSelection: function () {
    ...
    if (viewSelection) {
      ...
      $._PPP_.updateEventPanel(
        "No project items selected (or a bin was selected)."
      );
    }
  },
  ...
  adjustGraphicsWhiteLuminance: function () {
    ...
    if (supportedValues && currentGWL) {
      $._PPP_.updateEventPanel(
        "Graphics White Luminance was: " + currentGWL + "."
      );
      if (currentGWL === 100) {
      ...
      $._PPP_.updateEventPanel(
        "Graphics White Luminance changed to: " +
